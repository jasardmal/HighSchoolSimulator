init python:
    def time_callback():#constantly calculate the time
        ...
        if(hasattr(store, 'location')):
            if store.location == 1:
                store.stringlacation = "School"
            elif store.location == 2:
                store.stringlacation = "Home"
            elif store.location == 3:
                store.stringlacation = "Store"
            elif store.location == 4:
                store.stringlacation = "Park"
            else:
    ...
    #references for displaying info
    ...
    def Stats():#manages display of stats
        ui.frame(xfill=False, xminimum = 200, yminimum=None, xalign=0.0, yalign = 0.0)
        ui.vbox()
        ui.text("Intellegence %d" % (intellegence), xalign=0.0, size=20)
        ui.text("Charisma %d" % (charisma), xalign=0.0, size=20)
        ui.text("Stamina %d" % (stamina), xalign=0.0, size=20)
        ui.text("Stress %d" % (stress), xalign=0.0, size=20)
        ui.close()
        
    def Phase():#manages display of phases
        ui.frame(xfill=False, xminimum =None, yminimum=None, xalign=1.0, yalign = 0.05)
        ui.vbox()
        ui.text("%s" % (stringphase), xalign=0.0, size=20)
        ui.close()